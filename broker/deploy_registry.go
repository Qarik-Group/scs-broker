...
//func (broker *SCSBroker) deployRegistry(space ccv3.Space, serviceId string, desired int) ([]*result.PushApp, error) {
func (broker *SCSBroker) deployRegistry(serviceId string, instanceId string, desired int) ([]*result.PushApp, error) {
	pool := worker.New()
	defer pool.StopWait()
